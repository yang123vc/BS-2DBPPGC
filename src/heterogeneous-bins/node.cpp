...
double NODE::get_waste() const {
	return waste;
} //Devuelve el área libre.

void NODE::setPropUtil() {
	prop_utilizada = 1 - waste / (L * W);
} //Calcula la proporción de bin utilizada

double NODE::getPropUtil() {
	return prop_utilizada;

}
int NODE::getNumPiezas() {
	return num_piezas;
}

void NODE::setOpen(bool op) {
	bin_open = op;
}
...
void
NODE::setDim (const string &label, double stock_length, double stock_width)
{
  this->SizeID = label;
  this->L = stock_length;
  this->W = stock_width;
...
void NODE::include_piece(PIEZA p) {
...
void
NODE::initialize_node (const string &label, double stock_length,
		       double stock_width, int ID)
{
  setDim (label, stock_length, stock_width);
  setID (ID);
  num_piezas = 0;
  waste = L * W;
  prop_utilizada = 0;
...
void NODE::update_section(IRR_BIN &ib) //This function adds the pieces that are included in the irregular bin but not in the section of the bin.
		{
...
void NODE::update_ptos_sect(IRR_BIN &ib) {
	int i = 0;
	while (ib.getID() != section[i].getID())
		i++;	//We are now in the section of the bin that correspond to ib.
	...
	for (size_t j = 0; j < ptos_new->size(); j++)
...
void NODE::add_piece_SectToBin(IRR_BIN &ib) {
...
void NODE::set_pred(NODE &pred) {
	predecessor = &pred;
}

void NODE::set_ID_pzas_disp(vector<PIEZA> &pz) {
...
vector<int> NODE::get_IDdisp() const {
	return ID_pzas_disp;
}
void NODE::update_pzas_disp(int ID) {
	int pos = 0;
...
void NODE::set_globaleval(double of) {
	global_eval = of;
}

double NODE::get_globaleval() const {
	return global_eval;
}

void NODE::set_localevaluation(double of) {
	local_eval = of;
}

double NODE::get_localevaluation() const {
	return local_eval;
}

void NODE::FixUtil(double util) {
	prop_utilizada = util;
}
void NODE::FixDim(double w, double l) {
	W = w;
	L = l;
}

void NODE::CopyPiecesInSect() {
	int pos = 0;
