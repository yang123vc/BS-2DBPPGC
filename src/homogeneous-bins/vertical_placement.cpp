  ...
  while (!last_pzas.empty ()) //Not all the pieces have been placed.
    {
      ...
      if (length_gc < GRANDE)
	length_gc = HorGuillCut (ib_node, last_pzas, convex_hull, nodeW, nodeL,
				 best_rot, best_p_hc, best_i_hc, hcg); //Devuelve la altura total si la pieza se coloca a la dcha del actual convex hull.
