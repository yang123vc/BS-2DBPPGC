...
double
VertGuillCut (IRR_BIN &ib_node, vector<PIEZA*> last_pzas, PIEZA convex_hull,
	      double nodeW, double nodeL, double &best_rot, int &best_p,
	      int &best_i, EDGES &vcg);
void
  ...
  while (!last_pzas.empty ()) //Not all the pieces have been placed.
    {
      ...
      if (height_gc < GRANDE)
	height_gc = VertGuillCut (ib_node, last_pzas, convex_hull, nodeW, nodeL,
				  best_rot, best_p_vc, best_i_vc, vcg); //Devuelve la altura total si la pieza se coloca a la dcha del actual convex hull.
	  ...
	  for (int j = 0; j < (item.obtener_puntos ())->size (); j++)
	    {
	      ...
	      if (i_ptos[j].coordx < 0 - TOL || i_ptos[j].coordx > node_l + TOL
