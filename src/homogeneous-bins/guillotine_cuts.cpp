...
EDGES
IntersecGC (BIN bin, EDGES prev_gc, EDGES current_gc);
int
...
CalculateGC (BIN bin)
{
  vector<EDGES> *gc = bin.getGC ();
  vector<PUNTO> rbin; //'residual' bin, the bit of bin that is left once the guillotine cut is performed.
  PUNTO addrbin;
  //Add vertex of bin
  addrbin.coordx = 0;
  addrbin.coordy = 0;
  rbin.push_back (addrbin);
  addrbin.coordx = bin.getL ();
  addrbin.coordy = 0;
  rbin.push_back (addrbin);
  addrbin.coordx = bin.getL ();
  addrbin.coordy = bin.getW ();
  rbin.push_back (addrbin);
  addrbin.coordx = 0;
  addrbin.coordy = bin.getW ();
  rbin.push_back (addrbin);
  ...
  for (int i = 0; i < (*gc).size (); i++)
    {
      //Check intersection with edges of rbin.
      (*gc)[i] = IntersectBin ((*gc)[i], rbin);
      rbin = Redefine_IrrBin_withPieces (rbin, (*gc)[i]);
    }
  return *gc;
}
